...
def xss_rule(request):
    return (
        ...
        or (request.method == 'POST' and any("<script>" in value for value in request.form.values()))
    )
...
def path_traversal_rule(request):
    if "../" in request.path or "/../" in request.path or"../" in request.query_string.decode("utf-8") or "/../" in request.query_string.decode("utf-8"):
        return True
    return False
